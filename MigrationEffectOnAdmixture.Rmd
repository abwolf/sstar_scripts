...
mdls <- list.files('~/DATALab/SimulatedDemographic/Sstar/chr1_variable_ref/simulations/Sriram/', pattern = 'n1_0.1')
...
for (mdl in mdls){
    ...
    infile <- paste0('~/DATALab/SimulatedDemographic/Sstar/chr1_variable_ref/simulations/Sriram/',mdl)
    
    chr_list <- fread(paste0(infile,'/Sriram.chr_list.mod'), col.names = c('chr'), colClasses = c('character'))
...
for (mdl in mdls){
    ...
    chr_list <- fread(paste0(infile,'/Sriram.chr_list.mod'), col.names = c('chr'), colClasses = c('integer')) %>% separate(col = 'chr', into = c('chr','pool')) %>% rowwise() %>% mutate(chr=as.integer(chr)) %>% as.data.table()
...
plot.modern_migration <- 
  ...
  ################################
    ...
    scale_color_manual(values = c('EUR.IBDmix' = 'steelblue2','EUR.Sstar'='steelblue3','EAS.IBDmix'='darkorange','EAS.Sstar'='darkorange3')) +
    ...
    scale_y_continuous(breaks = seq(0,0.16,by = 0.02), labels=seq(0,0.16,by=0.02)) + 
...
plot.ancient_migration <- 
  ...
  ################################
    ...
    scale_color_manual(values = c('EUR.IBDmix' = 'steelblue3','EUR.Sstar'='cyan4','EAS.IBDmix'='orangered2','EAS.Sstar'='firebrick3')) +
    #scale_color_hue(l=50, c=100) +
