...
mdls <- list.files('~/DATALab/SimulatedDemographic/Sstar/chr1_variable_ref/simulations/Tenn/n1_0.1/', pattern = 'n1_0.1')
...
for (mdl in mdls){
    ...
    infile <- paste0('~/DATALab/SimulatedDemographic/Sstar/chr1_variable_ref/simulations/Tenn/n1_0.1/',mdl)
...
for (mdl in mdls){
    ...
    chr_list <- fread(paste0(infile,'/Tenn.chr_list'), col.names = c('chr'), colClasses = c('integer'))
    
    ###########
    bedfile.EAS <- fread(input = paste0("cat ",infile,"/IBDmixCalls/IntroSeq.ASN.D4E0.002.ALL.txt"),
    ...
    ###########
    bedfile.EUR <- fread(paste0('cat ',infile,'/IBDmixCalls/IntroSeq.EUR.D4E0.002.ALL.txt'),
                   col.names = c('Ind','chr','start','end','LOD1','LOD2'))
    bedfile.EUR[,len_bp:=end-start]
    ...
    bedfile.ALL <- rbind(bedfile.EAS, bedfile.EUR)
    bedfile.ALL.chr <- bedfile.ALL %>% group_by(chr) %>% summarise(admix = sum(as.numeric(len_bp))/1007/1000000) %>% as.data.table()
...
dt.IBDmix %>% setnames(c('mdl','admix.ALL','ci.ALL','sd.ALL','admix.EAS','ci.EAS','sd.EAS','admix.EUR','ci.EUR','sd.EUR','EAS:EUR','t.test.pval'))
...
plot.modern_migration <- 
  ggplot() + theme_bw() +
    ...
    ##############################
    geom_point(data = dt.Sstar.modern, aes(y=as.numeric(admix.EAS), x=mdl, color='EAS.Sstar'), size=5 ) +
    geom_errorbar(data = dt.Sstar.modern, aes(y=as.numeric(admix.EAS),
                                             ymin=as.numeric(ci.EAS.lwr), ymax=as.numeric(ci.EAS.upr),
                                             x=mdl, color='EAS.Sstar'), size=3 ) +
    geom_line(data = dt.Sstar.modern, aes(y=as.numeric(admix.EAS), x=mdl, color='EAS.Sstar', group=1), size=3 ) +
  
    geom_point(data = dt.Sstar.modern, aes(y=as.numeric(admix.EUR), x=mdl, color='EUR.Sstar'), size=5 ) +
    geom_errorbar(data = dt.Sstar.modern, aes(y=as.numeric(admix.EUR), 
                                             ymin=as.numeric(ci.EUR.lwr), ymax=as.numeric(ci.EUR.upr), 
                                             x=mdl, color='EUR.Sstar'), size=3 ) +
    geom_line(data = dt.Sstar.modern, aes(y=as.numeric(admix.EUR), x=mdl, color='EUR.Sstar', group=1), size=3 ) +
   ...
   ###############################  
    ...
    ##############################
    geom_point(data = dt.IBDmix.modern, aes(y=as.numeric(admix.EAS), x=mdl, color='EAS.IBDmix'), size=5 ) +
    geom_errorbar(data = dt.IBDmix.modern, aes(y=as.numeric(admix.EAS),
                                             ymin=as.numeric(ci.EAS.lwr), ymax=as.numeric(ci.EAS.upr),
                                             x=mdl, color='EAS.IBDmix'), size=3 ) +
    geom_line(data = dt.IBDmix.modern, aes(y=as.numeric(admix.EAS), x=mdl, color='EAS.IBDmix', group=1), size=3 ) +
  
    geom_point(data = dt.IBDmix.modern, aes(y=as.numeric(admix.EUR), x=mdl, color='EUR.IBDmix'), size=5 ) +
    geom_errorbar(data = dt.IBDmix.modern, aes(y=as.numeric(admix.EUR), 
                                             ymin=as.numeric(ci.EUR.lwr), ymax=as.numeric(ci.EUR.upr), 
                                             x=mdl, color='EUR.IBDmix'), size=3 ) +
    geom_line(data = dt.IBDmix.modern, aes(y=as.numeric(admix.EUR), x=mdl, color='EUR.IBDmix', group=1), size=3 ) +
...
#    scale_color_manual(breaks=c('ALL','EAS','EUR')) +
    ...
    scale_y_continuous(breaks = seq(0,0.14,by = 0.02), labels=seq(0,0.14,by=0.02)) + 
#    scale_y_continuous(trans = "log10", limits = c(NA,-2)) +
    coord_cartesian(ylim = c(0,0.14), expand = c(0)) +
    theme()
##################################
##################################
...
plot.ancient_migration <- 
  ggplot() + theme_bw() +
    ...
    geom_point(data = dt.Sstar.ancient, aes(y=as.numeric(admix.EAS), x=mdl, color='EAS.Sstar'), size=5 ) +
    geom_errorbar(data = dt.Sstar.ancient, aes(y=as.numeric(admix.EAS),
                                             ymin=as.numeric(ci.EAS.lwr), ymax=as.numeric(ci.EAS.upr),
                                             x=mdl, color='EAS.Sstar'), size=3 ) +
    geom_line(data = dt.Sstar.ancient, aes(y=as.numeric(admix.EAS), x=mdl, color='EAS.Sstar', group=1), size=3 ) +
  
    geom_point(data = dt.Sstar.ancient, aes(y=as.numeric(admix.EUR), x=mdl, color='EUR.Sstar'), size=5 ) +
    geom_errorbar(data = dt.Sstar.ancient, aes(y=as.numeric(admix.EUR), 
                                             ymin=as.numeric(ci.EUR.lwr), ymax=as.numeric(ci.EUR.upr), 
                                             x=mdl, color='EUR.Sstar'), size=3 ) +
    geom_line(data = dt.Sstar.ancient, aes(y=as.numeric(admix.EUR), x=mdl, color='EUR.Sstar', group=1), size=3 ) +
   ...
   ###############################  
    ...
    #                                          x=mdl, color='EUR'), size=3 ) +
    ##############################
    geom_point(data = dt.IBDmix.ancient, aes(y=as.numeric(admix.EAS), x=mdl, color='EAS.IBDmix'), size=5 ) +
    geom_errorbar(data = dt.IBDmix.ancient, aes(y=as.numeric(admix.EAS),
                                             ymin=as.numeric(ci.EAS.lwr), ymax=as.numeric(ci.EAS.upr),
                                             x=mdl, color='EAS.IBDmix'), size=3 ) +
    geom_line(data = dt.IBDmix.ancient, aes(y=as.numeric(admix.EAS), x=mdl, color='EAS.IBDmix', group=1), size=3 ) +
  
    geom_point(data = dt.IBDmix.ancient, aes(y=as.numeric(admix.EUR), x=mdl, color='EUR.IBDmix'), size=5 ) +
    geom_errorbar(data = dt.IBDmix.ancient, aes(y=as.numeric(admix.EUR), 
                                             ymin=as.numeric(ci.EUR.lwr), ymax=as.numeric(ci.EUR.upr), 
                                             x=mdl, color='EUR.IBDmix'), size=3 ) +
    geom_line(data = dt.IBDmix.ancient, aes(y=as.numeric(admix.EUR), x=mdl, color='EUR.IBDmix', group=1), size=3 ) +
